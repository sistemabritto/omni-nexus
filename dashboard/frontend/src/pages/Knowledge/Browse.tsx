...
const STATUS_COLORS: Record<string, string> = {
  ...
  pending: 'bg-white/5 text-[#98A2B3]',
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-3 flex-wrap">
        ...
        <button onClick={load} className="p-2 text-[#98A2B3] hover:text-[#D0D5DD] hover:bg-white/5 rounded-lg transition-colors" title="Refresh">
          <RefreshCw size={14} />
        </button>
      </div>
                ...
                <tr key={doc.id} className="border-b border-[#344054]/50 last:border-0 hover:bg-white/2 transition-colors">
                  <td className="px-4 py-3 max-w-[200px]">
                    <div className="flex items-center gap-2">
                      <FileText size={12} className="text-[#98A2B3] shrink-0" />
                  ...
                  <td className="px-4 py-3 text-xs text-[#98A2B3]">{doc.difficulty_level || '—'}</td>
                  <td className="px-4 py-3">
                    <div className="flex gap-1 flex-wrap max-w-[120px]">
                      {(doc.topics || []).slice(0, 2).map((t) => (
                        <span key={t} className="text-[10px] px-1.5 py-0.5 rounded bg-white/5 text-[#98A2B3]">{t}</span>
                      ))}
                      {(doc.topics?.length || 0) > 2 && <span className="text-[10px] text-[#98A2B3]">+{(doc.topics?.length || 0) - 2}</span>}
                    </div>
                  </td>
                  <td className="px-4 py-3 text-xs text-[#98A2B3]">{doc.pages_count ?? '—'}</td>
                  <td className="px-4 py-3 text-xs text-[#98A2B3]">{doc.chunks_count?.toLocaleString() ?? '—'}</td>
                  <td className="px-4 py-3">
                    <span className={`text-xs px-1.5 py-0.5 rounded ${STATUS_COLORS[doc.status] || 'bg-white/5 text-[#98A2B3]'}`}>
                      {doc.status}
                    </span>
                  </td>
                  <td className="px-4 py-3 text-xs text-[#98A2B3]">{new Date(doc.created_at).toLocaleDateString()}</td>
                  <td className="px-4 py-3">
                    {canManage && (
                      <button
      ...
      {confirmDeleteId && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
          <div className="bg-[#0C111D] border border-[#344054] rounded-xl w-full max-w-sm shadow-2xl p-6 text-center">
            ...
            <p className="text-xs text-[#98A2B3] mb-6">All chunks and embeddings for this document will be permanently deleted.</p>
