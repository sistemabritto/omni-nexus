...
- **"Rotina fantasma" é quase sempre o lock, não a rotina.** `good-morning`/`end-of-day` falhavam todo dia 07:00 com returncode=1, duration exata de ~120s, stdout=0 linhas. Não era o provider nem o prompt: os heartbeats `autopilot-*` acordavam às 07:00 segurando o `flock` cross-container (`orchestrator-bash.lock`) e a rotina — que usa `invoke_with_fallback` sem retry — esperava `ORCHESTRATOR_LOCK_WAIT_SECONDS` (120s), virava status="busy" e morria. Diagnosticar pelo padrão (duração idêntica dia após dia + hora fixa) apontava pro lock; rodar a rotina isolada no container (com o venv `/workspace/.venv/bin/python3`, não o system python sem `rich`) confirmou que sozinha funciona em 76s. A correção não foi "tornar a rotina mais rápida" — foi retry com backoff em cima da crise externa (`runner.py`). Lixo de diagnóstico: `/usr/bin/python3` do container não tem `rich`; o scheduler roda com `/workspace/.venv/bin/python3`. Reproduzir com o python errado dá `ModuleNotFoundError` e esconde o erro real.
- **O bot "desistir" era escolha de caller, não do motor.** `invoke_with_fallback` já gira cadeia de providers/modelos em 429; o que faltava era repetir a CHAMADA INTEIRA quando a crise é externa (busy/rede). Separar as duas camadas (`resilient_invoke` por cima de `invoke_with_fallback`) mantém callers antigos intactos (sem budget = chamada única) e centraliza o backoff. Detalhe que quase quebrou: a contagem de retry. `max_retries=N` = N tentativas EXTRAS após a 1ª (N+1 total); a fórmula `2**retries_done` só dá o backoff certo quando `retries_done` começa em 0 na primeira falha. E o texto parcial do NDJSON NÃO pode ser descartado na falha final — é o que transformaria um "Falhei" vazio numa resposta incompleta mas útil.
- **Configurações é onde o menu guarda o que ninguém abre — e vira dívida.** As abas Notificações (flag órfã em localStorage `evonexus.notifications.enabled`, lida num único lugar no AgentChat), Confiança (um toggle que pertencia aos providers) e Referência (CLAUDE.md num sub-tab) ocupavam 5 abas p/ 3 controles. Enxugar = remover as 3, embutir Usuários/Papéis/Auditoria (que só existiam como páginas standalone sem visibilidade p/ quem não tem permissão de sidebar) e Backups. Padrão que se repetiu na sessão: antes de remover uma UI, grepar a flag/endpoints nos dois lados — `evonexus.notifications.enabled` ainda é lido por `AgentChat.tsx` mesmo depois da aba sumir (funcionalidade segue, só perde o interruptor).
